...
final class IntesaPaymentView
{
    ...
    public function view(ShopOrder $order, string $locale): array
    {
        ...
        $orgOkUrl = $this->router->generate('site.checkout_completed_card_return.intesa', ['_locale' => $locale, 'token' => $order->getToken()], UrlGeneratorInterface::ABSOLUTE_URL);
        $orgFailUrl = $this->router->generate('site.checkout_completed_card_return.intesa', ['_locale' => $locale, 'token' => $order->getToken()], UrlGeneratorInterface::ABSOLUTE_URL);
