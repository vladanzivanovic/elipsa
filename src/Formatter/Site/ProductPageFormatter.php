...
final class ProductPageFormatter
{
    ...
    public function formatResponse(array $data): array
    {
        foreach ($data['images'] as &$image) {
            $image['image_link'] = $this->router->generate('app.image_show', ['name' => $image['fileName'], 'filter' => 'product_image']);
            $image['image_link_thumb'] = $this->router->generate('app.image_show', ['name' => $image['fileName'], 'filter' => 'product_image_thumb']);
